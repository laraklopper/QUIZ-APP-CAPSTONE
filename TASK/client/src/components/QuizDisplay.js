  ...
  const [currentScore, setCurrentScore] = useState(0);// State to store the user's score during the quiz
    ...
    const checkExistingScore = useCallback(async () => {
      try {
        ...
        //Conditional renderering to check if userScores existsnand is an array before calling find
        const existingScore = Array.isArray(result.userScores)
          ? result.userScores.find(score => score.name === quizName)// Find the existing score for the current quiz
          : null;
